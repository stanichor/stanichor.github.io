...
People like to point to Shapley values as a definition of fairness that even aliens would supposedly invent. And to be fair, Shapley values *do* have nice mathematical properties. But nothing compels agents to distribute gains according to them. The same is true of Nash bargaining solutions and other cooperative ideals. These ideals describe outcomes that arise under certain assumptions; they aren't moral requirements. (Even if they *were* moral requirements, nothing obligates an agent to be moral.) If an agent can gain more utility by ignoring such ideals, then given its preferences, game theory says it *should*. If someone is too weak or uncoordinated to demand their "fair share," it is entirely possible that they simply will not get what they "deserve."

## What game theory is good for

Game theory is useful for many things. It can predict how interactions will unfold (given certain conditions), help us design systems so that particular outcomes *do* occur (given certain conditions), determine the optimal action to take (given certain conditions and already-fixed preferences), and even help explain why we have the values that we do.
