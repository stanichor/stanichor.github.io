...
This suggests that the Big Five domains, as currently measured, are not unitary causal entities but instead aggregates of partially distinct lower-level constructs. Because specific items explain variance in outcomes even after accounting for the domain-level factor, it does not make sense to treat the Big Five as unified psychological traits. (To be fair, psychologists do acknowledge the existence of facets beneath the domains, though they rarely investigate them in their research.) As measured, the personality domains, along with their facets, function more as convenient statistical summaries than as coherent underlying traits.
...
In the common pathway model, genetic and environmental influences on the indicators operate through the latent construct[^common-pathway-elaboration]. In the independent pathway model, genetic and environmental factors act directly on the indicators, and the latent factor is simply a convenient statistical summary of the indicators[^pathway-misspecification].
...
These common pathway and independent pathway models have very different implications. Evidence generally supports a common latent genetic pathway for cognitive ability, while for personality traits such as Conscientiousness, the independent pathway model tends to fit better. This means, for example, that editing genes related to intelligence would primarily influence general cognitive ability[^common-pathway-explanation]. On the other hand, editing genes related to, say, Conscientiousness would have less predictable effects, since such edits could affect traits such as self-discipline, prudence, or even social desirability bias rather than a single unified Conscientiousness trait[^independent-pathway-explanation].
...
Ultimately, arguably the most important aspect of mediation is that it is a core requirement of a reflective factor model, the kind discussed throughout this post. In reflective factor models, the latent construct explains (that is, it *mediates*) the shared variation among indicators, such that any remaining correlation among indicators is negligible once the latent factor is fixed. Without this property, the very idea of a coherent latent trait begins to lose its footing and psychometrics falls apart.
...
[^common-pathway-elaboration]: An important implication is that when different external variables affect the indicators only through the same latent variable, their effects on the items must be proportional. For any two such variables, A and C, the influence of A on a given item is a scalar multiple $k$ of the influence of C on that item, with the same $k$ applying to all items that depend on that latent factor. This proportional pattern would be very unlikely if multiple distinct factors were acting directly on the items, because they would probably have different effects across items.
[^athleticism-caveat]: It depends on how homogeneous your indicators are. If the indicators measure different physiological systems (e.g., strength, flexibility, endurance, reaction time), as is usually the case when people discuss a general athleticism factor, independent pathways should almost always fit better. If the test battery is more homogeneous, for example consisting only of endurance tests, common pathways may sometimes fit reasonably well.
...
[^pathway-misspecification]: An independent pathway model may also fit better because your model of the latent variables is incorrect! For example, if there are two latent variable but your model contains only one, it may look as if genetic and environmental influences on an indicators are not mediated by the latent variable, because those influences can affect indicators separately (that is, *not* through the modeled latent variable). In reality, however, some indicators may load on the first latent variable while others load on the second. Your modeled variable is therefore a convenient statistical summary of the indicators, rather than a single latent construct through which their genetic and environmental influences are mediated (even though there do exist latent constructs that mediate the genetic and environmental influences).
