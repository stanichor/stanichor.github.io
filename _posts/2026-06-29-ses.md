...
<div style="text-align: center;">
  <figure> 
    <img src="/assets/images/ses/reflective-model.png" alt="Reflective SES model in which socioeconomic status causes income, education, and occupation." width="500" /> 
    <figcaption> 
...
<div style="text-align: center;">
  <figure> 
    <img src="/assets/images/ses/formative-model.png" alt="Formative SES model in which income, education, and occupation combine to form socioeconomic status." width="500" /> 
    <figcaption> 
...
<div style="text-align: center;">
  <figure> 
    <img src="/assets/images/ses/equal-weight-model.png" alt="Formative SES model in which income, education, and occupation receive equal weights when forming socioeconomic status." width="500" /> 
    <figcaption> 
...
The problem with the second method is that, by choosing the weights to predict a single external criterion, we're essentially just doing linear regression. Sure, this is useful if our goal is prediction, but the resulting score doesn't generalize: it's specific to the criterion we're predicting.

<div style="text-align: center;">
  <figure> 
    <img src="/assets/images/ses/external-criterion-model.png" alt="Formative SES model in which income, education, and occupation form socioeconomic status, which then predicts financial security." width="500" /> 
    <figcaption> 
...
A set of weights chosen to predict financial security isn't automatically a general measure of socioeconomic status and shouldn't be used as one.
...
<div style="text-align: center;">
  <figure> 
    <img src="/assets/images/ses/mimic-model.png" alt="MIMIC-style SES model in which income, education, and occupation form socioeconomic status, which then predicts financial security, neighborhood quality, and self-perceived class." width="500" /> 
    <figcaption> 
...
[^trivial]: For income, you still have to decide what exactly you want to measure: individual vs household income, current vs permanent income, or even income vs wealth. For education, while years of education is numerically convenient, it seems like what really matters are the credentials, which are on an ordinal scale, not a ratio scale.

[^reflective]: More flexible structural equation models relax this assumption by, for example, including correlated residuals, additional latent variables, cross-loadings, method factors, etc. The important part is that a reflective model can determine its parameters from the covariance structure of the indicators, whereas a formative model cannot determine its weights from that covariance structure.
